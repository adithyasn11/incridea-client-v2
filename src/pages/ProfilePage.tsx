...
import { useMutation, useQuery, type QueryFunction, type MutationFunction } from '@tanstack/react-query'
    ...
    <section className="space-y-6">
      ...
      <div className="card space-y-4 p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          >
            {requestResetMutation.isPending ? 'Sending reset link…' : 'Send password reset link'}
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4 py-6 backdrop-blur">
          <div className="w-full max-w-md space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl shadow-slate-950/60">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="muted">Security</p>
                <h3 className="text-lg font-semibold text-slate-50">Change password</h3>
              </div>
              <button
                type="button"
            ...
            <form className="space-y-4" onSubmit={(event) => void onSubmit(event)}>
              ...
              <div className="flex items-center gap-3">
                <button className="button" type="submit" disabled={changePasswordMutation.isPending}>
